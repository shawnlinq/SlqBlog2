...
class Post(db.Document):
    ...
    def save(self, allow_set_time=False, *args, **kwargs):
        ...
        # exp: convert markdown to html by markdown2 model and clean html tags
    ...
    def to_dict(self):
        post_dict = {'title': self.title, 'slug': self.slug, 'abstract': self.abstract, 'raw': self.raw,
                     'pub_time': self.pub_time.strftime('%Y-%m-%d %H:%M:%S'),
                     'update_time': self.update_time.strftime('%Y-%m-%d %H:%M:%S'), 'content_html': self.content_html,
                     'author': self.author.username, 'category': self.category, 'tags': self.tags,
